...
function activate(_) {
	...
	vscode.commands.registerCommand("blackbox.codeSearch", () => {
		if (currentPanel) {
			...
			currentPanel.webview.html = getCodeSearchView(currentPanel)
			function postMessage(obj) {
			...
			currentPanel.webview.onDidReceiveMessage(async (data) => {
				if (data.command === "searchCode") {
					const response = await (0, node_fetch_1.default)(
						"https://useblackbox.io/autocompletev3",
						{
							method: "POST",
							body: JSON.stringify({
					...
					postMessage({
						command: "showCode",
						codes: result.response
					})
					selectionFct('code search from vscode sidenavigation')
	...
	vscode.commands.registerCommand("blackbox.showCode", () => {
		if (codePanel) {
			...
			postMessage({ command: "showCode", code: codeReturn, language: codeLanguage })
		} else {
			function postMessage(obj) {
				codePanel.webview.postMessage(obj)
			}
			...
			codePanel.webview.html = getCodeHtml()
				...
				(function() {
					...
					function showCode(code, language){
						...
						code.forEach((arr, idx)=>{
							...
							if(comment.length > 0){
								const $comment = $('<div class="code-comment">'+comment+'</div>')
								$el.append($comment)
							}
							if(code.length > 0){
								const $code = $('<textarea class="code-snippet">'+code+'</textarea>')
								$el.append($code)
							}
							$holder.append($el)
							if(code.length > 0){
					...
					window.addEventListener('message', event => {
						...
						if(data.command == "showCode"){
							showCode(data.code, data.language)
						}
					})
					getCode()
				}())
